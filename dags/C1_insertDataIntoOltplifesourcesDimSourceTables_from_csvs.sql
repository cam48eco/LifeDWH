...
INSERT INTO [dbo].[#csvlist]([filenames], [depth], [csvfile]) EXEC master.dbo.xp_dirTree '/path/to/csvcommunities', 1, 1;
...
SET @csvpath= '/path/to/csvcommunities/' + (SELECT filenames FROM [dbo].[#csvlist] WHERE id = 1)
...
BEGIN
	...
	SET @csvpath = '/path/to/csvcommunities/' + (SELECT filenames FROM [dbo].[#csvlist] WHERE id = @Counter)
