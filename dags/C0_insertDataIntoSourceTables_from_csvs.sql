...
INSERT INTO [dbo].[#csvlist]([filenames], [depth], [csvfile]) EXEC master.dbo.xp_dirTree '/Path/to/csvs/directory', 1, 1;
...
SET @csvpath= '/Path/to/csvs/directory/' + (SELECT filenames FROM [dbo].[#csvlist] WHERE id = 1)
...
BEGIN
	...
	SET @csvpath = '/Path/to/csvs/directory/' + (SELECT filenames FROM [dbo].[#csvlist] WHERE id = @Counter)
