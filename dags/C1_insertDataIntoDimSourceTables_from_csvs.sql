...
INSERT INTO [dbo].[#csvlist]([filenames], [depth], [csvfile]) EXEC master.dbo.xp_dirTree '/home/pawelc/communities', 1, 1;
...
SET @csvpath= '/path/to/communities/' + (SELECT filenames FROM [dbo].[#csvlist] WHERE id = 1)
...
BEGIN
	...
	SET @csvpath = '/path/to/communities/' + (SELECT filenames FROM [dbo].[#csvlist] WHERE id = @Counter)
	-- Assign queries to be executed 
	SET @sqldrop= 'DROP TABLE [dbo].' + QUOTENAME(@tablename) + ';'
	SET @sqlcreate= 'CREATE TABLE [dbo].' + QUOTENAME(@tablename) + '([gmina_id] nvarchar(10), [gmina_name] nvarchar(50), [gmina_powiat_name] nvarchar(50), [gmina_region_name] nvarchar(50), [gmina_stat_code] nvarchar(50), [gmina_pid] nvarchar(50), [gmina_sid] nvarchar(50), [gmina_attrib1] nvarchar(50), [gmina_attrib2] nvarchar(50));'
